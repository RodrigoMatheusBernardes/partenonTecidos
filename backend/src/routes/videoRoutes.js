...
// ============================================================
// ROTA DE VÍDEOS – com correção robusta
// ============================================================
try {
  console.log('🔄 Importando videoRoutes...');
  let videoRoutes = require('./routes/videoRoutes');
  console.log('✅ videoRoutes importado com sucesso. Tipo:', typeof videoRoutes);

  // Se for um objeto com propriedade router, extrai a função
  if (videoRoutes && typeof videoRoutes === 'object' && videoRoutes.router) {
    videoRoutes = videoRoutes.router;
  }

  // Se ainda não for uma função, lança erro
  if (typeof videoRoutes !== 'function') {
    throw new Error('videoRoutes não exporta uma função middleware');
  }
