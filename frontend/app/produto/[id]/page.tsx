...
export default function ProdutoPage() {
  ...
  if (carregando) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="w-12 h-12 border-4 border-[#e8e3dc] border-t-[#0B1F33] rounded-full animate-spin" />
    </div>
  );
  ...
  if (erro || !produto) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4">
      ...
      <Link href="/loja" className="text-sm font-semibold text-dark-light hover:text-[#C5A880] transition-colors underline">
        Voltar para a loja
      </Link>
    </div>
  );
  ...
  return (
    <main className="min-h-screen bg-white">
      <div className="main-container py-24 md:py-32">
        ...
        {/* BREADCRUMB */}
        <nav className="flex items-center gap-2 text-sm text-text-light mb-8 md:mb-12" aria-label="Breadcrumb">
          <Link href="/" className="hover:text-[#C5A880] transition-colors">Home</Link>
          <ChevronRight className="w-3 h-3" strokeWidth={2} />
          <Link href="/loja" className="hover:text-[#C5A880] transition-colors">Loja</Link>
          {produto.categoria?.nome && (
            <>
              <ChevronRight className="w-3 h-3" strokeWidth={2} />
              <span className="hover:text-[#C5A880] transition-colors">{produto.categoria.nome}</span>
            </>
          )}
          <ChevronRight className="w-3 h-3" strokeWidth={2} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16">
                ...
                {produto.fotos.map((foto, idx) => (
                  <button
                    ...
                    className={`
                      ...
                      focus:outline-none focus:ring-2 focus:ring-[#0B1F33]
          ...
          <div className="flex flex-col gap-6">
            ...
            {/* CATEGORIA */}
            {produto.categoria?.nome && (
              <span className="text-xs font-semibold uppercase tracking-widest text-lead-gray">
                {produto.categoria.nome}
              </span>
            )}

            {/* NOME */}
            <h1 className="font-serif font-semibold text-3xl md:text-4xl text-lead-gray leading-tight">
              {produto.nome}
            </h1>

            {/* PREÇO */}
            <div className="flex items-baseline gap-3">
              <span className="text-3xl font-serif font-semibold text-metallic-navy">
        ...
        {/* AVALIAÇÕES */}
        <section className="mt-16 md:mt-20 pt-12 border-t border-gray-mid">
